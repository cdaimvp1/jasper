...
|---|---|---|
| **No Teams sub-session boundaries** | Teams anchors only at the chat-container level; no `replyToId`-based or gap-based session splitting exists. | New: the sessionizer logic from Blueprint §7.3, Teams branch only. Calendar/Outlook/SharePoint sessionization can wait — they're lower-volume and (for Outlook) already effectively one-session-per-conversation, which the Blueprint itself says is fine as a starting point. |
...
| **Contract/order/document ID beyond PR/PO not extracted** | `REFERENCE_ID_RE` is literally `(?:PR\|PO)\d{4,}` — no CLM/SOW/MSA pattern. | New regex + reuse of the *existing* scoring/veto machinery (same function, wider pattern). Cheap, because the hard part (what to do with a matched reference) is already built. |
