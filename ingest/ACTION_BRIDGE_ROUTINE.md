...
4. **Do the action:**
   - **First, for EVERY `action_kind` (not just `review_contract`/`contract_review`) — check
     whether a real skill is registered under it** (task #112, 2026-08-04: Marc's explicit ask
     is that the system be able to run ANY of the registered skills on user confirmation/request,
     not just the couple with a dedicated button. The UI's "Run a skill" picker at
     `GET /api/skills` sends the skill's own registry key straight through as `action_kind`, so
     this check is what actually makes an arbitrary skill runnable, not the button that
     triggered it):
     ```
     python -c "import skills_registry; print(skills_registry.get_skill_for_action('<action_kind>'))"
     ```
     **Registry-driven — never hardcode a specific skill's name here; this file must stay
     correct no matter which skills are registered, or none.**
     ...
     **If a skill is registered** (a dict comes back, with a real `skill_dir` that exists on
     ...
     actual process end to end against the real linked attachment (if the action needs one) -
     do not substitute a lighter freeform pass, and do not assume what the skill does from its
     name alone. If the skill's `SKILL.md` names a foundation/dependency skill (e.g. shared
     brand/guardrail assets), it lives as a sibling directory next to it under the same
     ...
     **If nothing is registered** (`None` comes back), fall back to whichever of these named
     kinds matches, or the honest failure path in step 6 if none do:
   ...
   - `review_contract` / `contract_review` with no skill registered — the original ad-hoc
     behavior: read the linked document, produce concrete redline/risk notes (specific clauses,
     specific concerns), not a generic checklist.
